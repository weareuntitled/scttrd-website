...
export default async (request: Request, context: { next: () => Promise<Response> }) => {
  ...
  if (!password) return new Response('Not configured', { status: 503 });
  const cookies = request.headers.get('cookie') || '';
  if (cookies.includes('scttrd_press=1')) return context.next();
  if (request.method === 'POST') {
    const form = await request.formData();
    if (String(form.get('password') || '') === password) {
      const res = Response.redirect(url.href, 303) as unknown as Response;
      res.headers.set('Set-Cookie', 'scttrd_press=1; Path=/styleguide; Max-Age=86400; SameSite=Lax');
      return res;
    }
    return new Response(loginHtml('SCTTRD Press Pack', 'Falsches Passwort'), { status: 401, headers: { 'Content-Type': 'text/html' } });
  }
  return new Response(loginHtml('SCTTRD Press Pack'), { status: 401, headers: { 'Content-Type': 'text/html' } });
};
